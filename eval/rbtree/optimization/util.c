...
struct inode* malloc_inode(int mode, unsigned maj, unsigned min)
{
    ...
    if (mode != DIR_MODE) {
        ...
        // ret->prealloc_count = 0;
        // ret->prealloc_tree.rb_node = NULL;
    } else {
...
void dispose_inode(struct inode* inum)
{
    if (inum->mode != DIR_MODE) {
        ...
        // unsigned i;
        // for (i = 0; i < inum->prealloc_count; i++) {
        //     free(inum->prealloc_data[i]);
        // }
        // inum->extents = NULL; // Ensure it's set to NULL after freeing
        // // Free the prealloc extent rb_tree if it exists
        // if (inum->prealloc_tree.rb_node) {
        //     struct rb_node *node, *next;
            
        //     // Post-order traversal to free all nodes
        //     for (node = rb_first(&inum->prealloc_tree); node; node = next) {
        //     // Store next node before freeing current
        //     next = rb_next(node);
            
        //     // Free the prealloc data structure containing this node
        //     Prealloc *prealloc = rb_entry(node, Prealloc, rb_node);
        //     free(prealloc);
        //     }
            
        //     // Clear the root pointer
        //     inum->prealloc_tree.rb_node = NULL;
        // }
    } else {
        free(inum->dir);
    }
...
{
    unsigned char* ret;
    ret = malloc(num * PG_SIZE);
    memset(ret, 0, num * PG_SIZE);
    return ret;
}
