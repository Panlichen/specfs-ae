...
{
#ifdef LOCK_COUPLING
    mcs_node_t* cur = buf->hd;
    buf->mutex = 0;
    assert(cur);
    mcs_mutex_unlock(buf->impl, cur);
    free(cur);
#endif
    return;
}
...
struct inode* malloc_inode(int mode, unsigned maj, unsigned min)
{
    ...
    if(mode != DIR_MODE)
    {
        ret->file = malloc(sizeof(struct indextb));
        memset(ret->file, 0, sizeof(struct indextb));
    }
...
void dispose_inode(struct inode* inum)
{
    ...
    if(inum->mode != DIR_MODE)
    {
        ...
        brels(inum->file);  //  [TODO] added manually
        free(inum->file);
    }
